...
void MainWindow::onActionCopyAsGrpcurlTriggered() {
    ...
    stream << "grpcurl -d '";
    stream << QString(request->getBody()).replace('\n', "");
    stream << "'";
    ...
    for (int i = 0; i < protocolTreeModel->rowCount(QModelIndex()); i++) {
        ...
        if (request->getMethod().isChildOf(fileDescriptor)) {
            ...
            if (imports.isEmpty()) {
                // grpcurlのリクエストには、少なくとも1つのimport pathが必須
                stream << " -import-path '";
    ...
    if (const auto &server = request->getServer(); server) {
        if (server->useTLS) {
            ...
            if (certificate) {
                // TODO: escape
                if (!certificate->rootCertsPath.isEmpty()) {
                    stream << " -cacert '";
                    stream << certificate->rootCertsPath;
                    stream << "'";
                }
                if (!certificate->privateKeyPath.isEmpty()) {
                    stream << " -key '";
                    stream << certificate->privateKeyPath;
                    stream << "'";
                }
                if (!certificate->certChainPath.isEmpty()) {
                    stream << " -cert '";
                    stream << certificate->certChainPath;
                    stream << "'";
                }
            }
        } else {
    ...
    auto requestPath = QString::fromStdString(request->getMethod().getRequestPath());
